...
class User(Base):
    ...
    # 관계 설정
    patrol_users: Mapped[list["PatrolUser"]] = relationship("PatrolUser", back_populates="user")
...
class Patrol(Base):
    ...
    end_time: Mapped[datetime.datetime] = mapped_column(DateTime, nullable=True)  # 순찰 종료 시간
    memo: Mapped[str] = mapped_column(String(500), nullable=True)  # 메모
    active: Mapped[bool] = mapped_column(Boolean, default=True)  # 순찰 중인지 여부

    # 관계 설정
    patrol_users: Mapped[list["PatrolUser"]] = relationship("PatrolUser", back_populates="patrol", cascade="all, delete-orphan")
...
class PatrolUser(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    patrol_id: Mapped[int] = mapped_column(Integer, ForeignKey("patrol.id", ondelete="CASCADE"))
