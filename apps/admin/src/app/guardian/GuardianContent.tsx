...
export function GuardianContent() {
    ...
    useEffect(() => {
        fetch('https://uscode-silverguardian-api-627770884882.europe-west1.run.app/users')
            .then((e) => e.json())
        ...
        >
            ...
            <td style={{ padding: '12px 8px', fontSize: '14px' }}>
                <Input defaultValue={g.name} onBlur={(e) => {
                    ...
                    fetch(`https://uscode-silverguardian-api-627770884882.europe-west1.run.app/users/${g.id}`, {
                        method: 'PUT',
                        body: JSON.stringify({
                            name: e.target.value,
                            id: g.id,
